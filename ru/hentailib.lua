...
-- HentaiLib — 18+ манга на русском (LibGroup, api.cdnlibs.org).
-- Клон MangaLib с siteId=4 для взрослого контента.
-- Авторизация: Bearer Token (рекомендуется для доступа к 18+ контенту).
...
local function fetchChapterPages(chapterUrl)
  ...
  local infoR = http_get(apiBase .. slug, { headers = buildHeaders() })
  ...
  if data.pages and type(data.pages) == "table" then
    ...
    for _, page in ipairs(data.pages) do
      local url = page.url
      if url and url ~= "" then
        if not url:find("://") then url = "https://img3h.hentaicdn.org" .. url end
