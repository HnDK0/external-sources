...
-- MintManga (GroupLe) — 18+ манга на русском.
...
local function fetch(url)
    ...
    if not r.success then
        if r.code == 403 then
            log_error("mintmanga: 403 — требуется авторизация: " .. url)
        ...
        elseif r.code == 404 then
            log_error("mintmanga: 404 — страница не найдена: " .. url)
            if show_error then show_error("Ошибка загрузки", "Страница не найдена (404).") end
        else
            log_error("mintmanga: HTTP " .. (r.code or "?") .. " — " .. url)
...
local function fetchMangaDetails(bookUrl)
    ...
    if not slug then log_error("mintmanga: no slug from " .. bookUrl); return nil end
    if _mangaCache[slug] then return _mangaCache[slug] end
    local body = fetch(bookUrl)
    if not body then log_error("mintmanga: fetch failed for " .. bookUrl); return nil end
    log_error("mintmanga: fetched " .. #body .. " bytes for " .. bookUrl)
...
function getChapterList(bookUrl)
    ...
    if body:find("viewSettings", 1, true) and body:find("blockedForAnonymous", 1, true) and not body:find("window.current_user_id", 1, true) then
        log_error("mintmanga: требуется авторизация — blockedForAnonymous: " .. bookUrl)
...
function getPageList(html, url)
    ...
    while entryStart <= #pageArray do
        ...
        if a and b and c then
            ...
            -- ponytail: DDoS Guard блокирует URL с query params (?t=...&h=...)
            -- reader JS стрипает их перед загрузкой — мы тоже
