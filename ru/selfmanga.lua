...
-- SelfManga (GroupLe) — пользовательская манга на русском.
...
local function fetch(url)
    ...
    if not r.success then
        ...
        elseif r.code == 404 then
            ...
            log_error("selfmanga: HTTP " .. (r.code or "?") .. " — " .. url)
...
local function fetchMangaDetails(bookUrl)
    ...
    if not slug then log_error("selfmanga: no slug from " .. bookUrl); return nil end
    if _mangaCache[slug] then return _mangaCache[slug] end
    local body = fetch(bookUrl)
    if not body then log_error("selfmanga: fetch failed for " .. bookUrl); return nil end
    log_error("selfmanga: fetched " .. #body .. " bytes for " .. bookUrl)
...
function getChapterList(bookUrl)
    ...
    if body:find("Запрещена публикация произведения по копирайту", 1, true) then
        log_error("selfmanga: лицензировано — главы удалены: " .. bookUrl)
    ...
    if body:find("viewSettings", 1, true) and body:find("blockedForAnonymous[^}]*true", 1) and not body:find("window.current_user_id", 1, true) then
        log_error("selfmanga: требуется авторизация — blockedForAnonymous=true: " .. bookUrl)
